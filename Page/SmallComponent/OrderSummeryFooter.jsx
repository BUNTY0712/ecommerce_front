...
const OrderSummeryFooter = () => {
  const { bill, emailData } = useSelector((state) => state.ui);
    ...
    const handleSubmit = async () => {
        dispatch(setEmailData({
            ...emailData,
            name: "suraj",
            email: "kr.suraj1602@gmail",
        }))
       await sendEmail(emailData, dispatch, navigation);
  ...
  return (
      <View style={styles.container}>
          ...
          <Text style={styles.buttoncartText}>₹ {bill }</Text>
            ...
            <Text style={styles.buttonbuyText}>Continue</Text>
          </TouchableOpacity>
          </View>
    </View>
  );
